...
class LinearWarmupAndNoDecayLR(LinearWarmupLR):
    ...
    def _decay_step(self) -> list[float]:
        ...
        ---------
        float: 
            The learning rate for the current warmup step.
        """
...
class LinearWarmupAndInverseSqrtDecayLR(LinearWarmupLR):
    ...
    def _decay_step(self) -> list[float]:
        ...
        ---------
        float: 
            The learning rate for the current decay step.
        """
        scale = (self.last_epoch + 2) / (self.last_epoch + 1)
...
class LinearWarmupCosineAnnealingDecay(LinearWarmupLR):
    """
    ...
    cosine_annealing_steps (int): 
        The number of cosine annealing steps.
    min_lr (float): 
    ...
    **kwargs: 
    ...
    def _decay_step(self) -> list[float]:
        return [
            self.min_lr + (x - self.min_lr) * (1 + math.cos(math.pi * (self.last_epoch - self.warmup_epochs) / self.cosine_annealing_steps)) / 2
